...
export default async function listSystemDesigns(req, res) {
  ...
  const { rows } = await db.query(
    ...
    "SELECT id, title, slug, nodes, edges, type, tags, is_public, description, pattern, difficulty, created_at FROM system_designs WHERE user_id = $1 AND is_public = false ORDER BY created_at DESC LIMIT 60",
    [owner],
  );
