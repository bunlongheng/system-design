...
export default async function listSystemDesigns(req, res) {
  ...
  const { rows } = await db.query(
    // Owner's own working index = PERSONAL diagrams only (is_public = false). The
    // public showcase demos live on /demo, so they don't clutter the logged-in home.
    "SELECT id, title, slug, nodes, edges, type, tags, is_public, description, pattern, difficulty, created_at FROM system_designs WHERE user_id = $1 AND is_public = false AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 60",
    [owner],
  );
