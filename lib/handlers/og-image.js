  ...
  try {
    if (name) {
      const { rows } = await db.query(
        `SELECT ${COLS} FROM system_designs WHERE slug = $1 AND is_public = true AND deleted_at IS NULL LIMIT 1`,
        [name],
      );
      design = rows[0] || null;
    } else if (id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)) {
      const { rows } = await db.query(
        `SELECT ${COLS} FROM system_designs WHERE id = $1 AND is_public = true AND deleted_at IS NULL`,
        [id],
      );
      design = rows[0] || null;
    }
