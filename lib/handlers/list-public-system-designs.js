...
export default async function listPublicSystemDesigns(req, res) {
  ...
  const { rows } = await db.query(
    "SELECT id, title, slug, nodes, edges, type, tags, is_public, description, pattern, difficulty, created_at FROM system_designs WHERE is_public = true AND slug = ANY($1) ORDER BY difficulty ASC NULLS LAST, created_at DESC LIMIT 12",
    [DEMO_SLUGS],
  );
