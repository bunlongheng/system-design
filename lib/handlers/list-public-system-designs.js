...
export default async function listPublicSystemDesigns(req, res) {
  ...
  const { rows } = await db.query(
    "SELECT id, title, slug, nodes, edges, type, tags, is_public, description, created_at FROM system_designs WHERE is_public = true ORDER BY created_at DESC LIMIT 24",
  );
