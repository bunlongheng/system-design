...
export default async function systemDesignById(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    // Where the owner dragged a step badge to. Auto-placement keeps a badge off
    // its own node but cannot see the other badges, so on a dense diagram two
    // can still collide - these are the manual corrections. Merged BY EDGE ID
    // into the stored edges so a stale client cannot drop labels or endpoints.
    if (Array.isArray(body.edges)) {
      const offsets = new Map();
      for (const e of body.edges) {
        if (!e || typeof e.id !== "string") continue;
        const o = e.labelOffset;
        if (o && Number.isFinite(o.dx) && Number.isFinite(o.dy)) {
          offsets.set(e.id, { dx: Math.round(o.dx), dy: Math.round(o.dy) });
        } else {
          offsets.set(e.id, null); // reset back to the computed spot
        }
      }
      ...
      const merged = (cur[0].edges || []).map((e, i) => {
        ...
        // Strip any existing offset, then re-add only if there is a new one.
        const rest = { ...e };
      ...
      if (rows.length === 0) return res.status(404).json({ error: "Not found" });
      return res.status(200).json({ id: rows[0].id, moved: merged.filter((e) => e.labelOffset).length });
    }
