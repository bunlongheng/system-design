...
export default async function systemDesignById(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const { rows } = await db.query(
      "SELECT id, title, slug, nodes, edges, type, tags, is_public, created_at FROM system_designs WHERE id = $1",
      [id],
    );
    if (rows.length === 0) return res.status(404).json({ error: "Not found" });
    // Private diagrams are visible only to the owner; hide as 404 otherwise so a
    // private id can't be probed.
    if (!rows[0].is_public && !(await authorizeOwner(req))) {
