...
export default async function sharePage(req, res) {
  ...
  const send = (html, hit) => {
    // Says whether this response came from the share function at all, and whether
    // it found a design. Without it, a static index.html and a fallback from here
    // are indistinguishable from the outside.
    res.setHeader("x-sd-share", hit ? "hit" : "miss");
  ...
  if (!name) return send(shell, false);
  ...
  try {
    ...
    return send(shell, false);
  }
  if (!rows.length) return send(shell, false);
